<template>
	...
	<div class="container xl:max-w-6xl mx-auto mt-12">
		...
		<div class="mx-auto">
			<table class="mt-8 table-auto border-collapse rounded-tl-lg rounded-tr-lg bg-zinc-300 w-full border-zinc-400">
				...
				<tbody v-if="listaAgendamentos" class="rounded-lg">
					<tr v-for="(line, index) in listaAgendamentos" :key="index"
						:class="index % 2 === 1 ? 'bg-zinc-200' : 'bg-zinc-100'" class="py-2">
						<td class="px-2 py-2 capitalize">{{ line.corretor_id }}</td>
						<td class="px-2 py-2 border-l border-black capitalize">{{ line.cliente_id }}</td>
	...
	<ModalAgendamento :open="isModalOpened" :isModalOpened="isModalOpened" :corretorList="corretorList"
		:listaAgendamentos="listaAgendamentos" @close="isModalOpened = false" @cadastrarAgendamento="cadastrarAgendamento"
		class="transition-opacity" name="agendamentoModal">
	</ModalAgendamento>
